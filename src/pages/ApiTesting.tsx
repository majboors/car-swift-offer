...
const ApiTesting = () => {
  ...
  const identifyCar = async () => {
    ...
    try {
      ...
      // Call the first API
      const response = await fetch('https://car.applytocollege.pk/get-car-name', {
        method: 'POST',
        body: formData,
      });
  ...
  const getCarDetails = async () => {
    if (!selectedImage || !carIdentification) {
      toast({
    ...
    try {
      ...
      // Construct car name with make, model and year
      const carName = `${carIdentification.make} ${carIdentification.model} ${modelYear}`;
      formData.append('car_name', carName);
      
      // Call the second API
      const response = await fetch('https://car.applytocollege.pk/get-car-details', {
        method: 'POST',
        body: formData,
      });
      ...
      toast({
        title: "Error Getting Car Details",
        description: error.message || "An unexpected error occurred",
        variant: "destructive",
      });
    } finally {
  ...
  // Create listing using the retrieved car details
  const createListing = () => {
    ...
    // Prepare the description with tags and specifications
    let description = '';
    ...
    // and state for complex data like features
    navigate(
                  ...
                  <CardContent className="pt-6 space-y-6">
                    <div className="text-center">
                      ...
                      <p className="text-gray-600">
                        Complete car details retrieved from API
                      </p>
                    </div>
